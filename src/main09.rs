    ...
    fn update(&mut self, message: Message) -> Command<Message>  {
        match message {
            ...
            Message::CopyPressed => {
               ...
               self.mess_color = colorout;
               if errcode == 0 {
                   Command::perform(asynccopy::asynccopy(self.dir_value.clone(),self.outdir_value.clone(), self.mergescrol_value.clone()), Message::copydone)
               } else {
                   Command::none()
               }
            }
